...
E2E-encrypted attachments (inline image previews, save-to-Downloads), and a per-chat
settings sheet (safety-number verification + disappearing-messages picker). The sealed
...
* Call echo/noise: Android captures via the platform `VOICE_COMMUNICATION` path with
  ...
  source of the speaker→mic feedback static); desktop capture runs always-on RNNoise
  but relies on the OS audio stack (PipeWire/CoreAudio/WASAPI) for echo cancellation
  when using loudspeakers — prefer a headset.
* Voice recording/calls are compile-verified everywhere but mic capture is
  hardware-dependent: exercise on a real device (Android prompt flow) and a desktop
  with a microphone. Android call audio needs the RECORD_AUDIO runtime grant — the app
  prompts before opening the mic.
...
* Group chat UI.
* Calls (see Known limitations — design doc first).
